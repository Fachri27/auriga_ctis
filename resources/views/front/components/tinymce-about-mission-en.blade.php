<div x-show="lang === 'en'" x-data="{
    about_mission_en: @entangle('mission_en'),
    initEditor() {
        let self = this;
        if (tinymce.get('about_mission_editor_en')) tinymce.get('about_mission_editor_en').remove();
        tinymce.init({
            target: this.$refs.about_mission_editor_en,
            ...
            setup(editor) {
                editor.on('init', () => {
                    editor.setContent(self.about_mission_en || '');
                });
                editor.on('change keyup', () => {
                    self.about_mission_en = editor.getContent();
                });
                    ...
                    onAction: function() {
                        editor.insertContent(`
                                    <figure class='media-caption'>
                                        ...
                                        <figcaption class='media-caption-text'>Caption video</figcaption>
                                    </figure>
                                            `);
                    }
                });
                    ...
                    onAction: function() {
                        editor.insertContent(`
                                    <figure class='media-caption'>
                                        ...
                                        <figcaption class='media-caption-text'>Image caption</figcaption>
                                    </figure>
                                            `);
                    }
                });
            ...
            file_picker_callback(callback, value, meta) {
                ...
                cmsURL += meta.filetype === 'image' ? '&type=image' : meta.filetype === 'media' ? '&type=video' : '&type=file';
    ...
    <label class="block font-medium mb-1">Mission (EN)</label>
    <textarea x-ref="about_mission_editor_en" id="about_mission_editor_en"></textarea>
</div>
